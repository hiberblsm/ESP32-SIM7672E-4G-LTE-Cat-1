/*
 * 04_UDPTest.h — ESP32-S3 + SIM7672E UDP Soket Testi
 *
 * Bu sınıf YALNIZCA 04_UDPTest için gereken fonksiyonları içerir:
 ...
 *   - UDP soket (udpConnect/udpSend/udpReceive/udpClose)
 *
 * UDP formatı: her paket tek satır JSON + '\n'
 * Auth: JSON body içinde "token" alanı (header değil!)
 *
 * UDP gönderimi: AT+CIPSEND=<id>,<len>,"<host>",<port> (uzak adres zorunlu)
 * UDP alımı: +IPD<id>,<len>:<data> veya +IPUDP<id>,<len>:<data>
 *
 * Diğer örneklerin (TCP/MQTT/SMS/DTMF) fonksiyonları BURADA YOKTUR.
...
public:
    ...
    UdpTest(uint8_t txPin      = SIM7672_TX_PIN,
    ...
    // UDP soket
    bool   udpConnect(const String &host, uint16_t port);
...
private:
    ...
    bool _udpSend(const uint8_t *buf, size_t len, const String &host, uint16_t port);
