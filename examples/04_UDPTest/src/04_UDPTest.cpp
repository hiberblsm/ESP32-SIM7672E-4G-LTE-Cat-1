/*
 * 04_UDPTest.cpp — ESP32-S3 + SIM7672E UDP Soket Testi
 *
 * Yalnızca UDP testi için gerekli fonksiyonları içerir.
 * Token HTTP üzerinden alınır, veri UDP üzerinden gönderilir.
 * ESP32-S3 üzerinde UART2 (Serial2) kullanılır.
 */

#include "04_UDPTest.h"

// ==================== KURUCU & BAŞLATMA ====================

UdpTest::UdpTest(uint8_t txPin, uint8_t rxPin, uint8_t pwrKeyPin, uint8_t resetPin)
    : _txPin(txPin), _rxPin(rxPin), _pwrKeyPin(pwrKeyPin), _resetPin(resetPin),
      _debugEnabled(true), _netOpen(false), _udpHost(""), _udpPort(0)
{
    _serial = &Serial2;
}

bool UdpTest::begin(long baud) {
    if (_serial) {
        _serial->end();
        delay(30);
    }
...
String UdpTest::sendAT(const String &cmd, uint32_t timeoutMs) {
    clearBuffer();
    _serial->println(cmd);
...
bool UdpTest::sendATExpect(const String &cmd, const String &expected,
                            uint32_t timeoutMs) {
    return sendAT(cmd, timeoutMs).indexOf(expected) >= 0;
}

bool UdpTest::waitForResponse(const String &expected, uint32_t timeoutMs) {
    char   buf[AT_BUF_SIZE];
    size_t pos = 0;
    unsigned long start = millis();
...
void UdpTest::_exitDataModeAndDrain() {
    clearBuffer();
    _serial->write(0x1B);
    delay(250);
    clearBuffer();
...
bool UdpTest::initGPRS(const String &apn,
...
void UdpTest::httpTerm() {
    sendAT("AT+HTTPTERM", 2000);
}

bool UdpTest::httpSetUrl(const String &url) {
    String cmd = "AT+HTTPPARA=\"URL\",\"" + url + "\"";
    return sendATExpect(cmd, "OK");
}

bool UdpTest::httpSetContentType(const String &ct) {
    String cmd = "AT+HTTPPARA=\"CONTENT\",\"" + ct + "\"";
    return sendATExpect(cmd, "OK");
}

bool UdpTest::httpData(const String &body) {
...
int UdpTest::httpAction(int method) {
    char buf[64] = {0};
    uint8_t pos = 0;
    unsigned long start = millis();
...
String UdpTest::httpRead(int maxLen) {
...
bool UdpTest::getToken(String &token, String &clientId) {
    ...
    // HTTP bittikten sonra UDP yığınını yeniden aç
...
bool UdpTest::_cipConnect(const char *type,
...
bool UdpTest::_udpSend(const uint8_t *buf, size_t len,
                       const String &host, uint16_t port) {
    // UDP: uzak adres ve port gönderimde zorunludur
...
String UdpTest::_cipReceive(uint32_t timeoutMs) {
    char   rawBuf[512];
    size_t rawLen = 0;
    unsigned long start = millis();
    while (millis() - start < timeoutMs) {
    ...
    // UDP'de veri +IPD<id>,<len>:... veya +IPUDP<id>,<len>:... ile gelir
    char marker[16];
    snprintf(marker, sizeof(marker), "+IPD%d,", CIP_IDX);
    char *ipdPtr = strstr(rawBuf, marker);
    if (!ipdPtr) {
        snprintf(marker, sizeof(marker), "+IPUDP%d,", CIP_IDX);
        ipdPtr = strstr(rawBuf, marker);
    }
    if (ipdPtr) {
...
bool UdpTest::udpConnect(const String &host, uint16_t port) {
    debugPrint("UDP baglanti: " + host + ":" + String(port));
    _udpHost = host;
    _udpPort = port;
    // SIM7672E'de UDP için localPort parametresi ZORUNLUDUR (0 = otomatik).
    // Bu, AT+CIPOPEN=0,"UDP",host,port,0 komutunu üretir.
    if (_cipConnect("UDP", host, port, 0)) {
        debugPrint("UDP baglanildi!");
        return true;
    }
    debugPrint("HATA: UDP baglanti basarisiz!");
    return false;
}

bool UdpTest::udpSend(const String &data) {
    if (_udpHost.length() == 0) {
        debugPrint("HATA: Once udpConnect cagrilmali!");
        return false;
    }
    if (_udpSend((const uint8_t *)data.c_str(), data.length(), _udpHost, _udpPort)) {
        return true;
    }
    debugPrint("HATA: UDP gonderim basarisiz!");
    return false;
}

String UdpTest::udpReceive(uint32_t timeoutMs) {
    return _cipReceive(timeoutMs);
}

bool UdpTest::udpClose() {
    bool ok = sendATExpect("AT+CIPCLOSE=" + String(CIP_IDX),
                            "+CIPCLOSE:", 5000);
    debugPrint("UDP kapatildi");
    return ok;
}

bool UdpTest::isUDPConnected() {
...
void UdpTest::debugPrint(const String &msg) {
    if (_debugEnabled) {
...
void UdpTest::setDebug(bool enabled) {
