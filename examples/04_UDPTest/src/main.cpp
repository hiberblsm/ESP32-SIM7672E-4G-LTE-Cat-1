/*
 * main.cpp — ESP32-S3 + SIM7672E UDP Soket Testi (PlatformIO)
 *
 * Bağımsız kütüphane: 04_UDPTest.h / 04_UDPTest.cpp
 * Test Sunucusu: test.hibersoft.com.tr:2886 (UDP)
 *
 * Akış:
 ...
 *   4. UDP bağlan (port 2886)
 *   5. 1 dakika boyunca UDP ile JSON paketi gönder
 *
 * UDP formatı: her paket tek satır JSON + '\n'
...
UdpTest gsm(17, 16, 4, 5);  // txPin, rxPin, pwrKeyPin, resetPin

// ====== AYARLAR ======
const char APN_STR[]   = "internet";
const char UDP_HOST[]  = "test.hibersoft.com.tr";
const char DEVICE_ID[] = "esp32-udp";
...
void setup() {
    ...
    printSeparator();
    Serial.println("  ESP32-S3 + SIM7672E UDP Test");
    ...
    if (choice == "3") {
        ...
        if (authToken[0] == '\0') {
            ...
            // Manuel modda clientId de sorulur; kullanıcı ne girdiyse o kullanılır
    ...
    printSeparator();
    if (authToken[0] == '\0') {
        Serial.println("  Token yok — UDP veri gonderilemez!");
        printSeparator();
        while (1) delay(1000);
    }

    // UDP bağlan
    Serial.print("UDP baglaniyor: ");
    Serial.print(UDP_HOST);
    Serial.print(':');
    Serial.println(UDP_PORT);
    if (!gsm.udpConnect(UDP_HOST, UDP_PORT)) {
        Serial.println("[FAIL] UDP baglanti kurulamadi!");
        gsm.closeGPRS();
        while (1) delay(1000);
    }
    Serial.println("[PASS] UDP baglanildi!");

    Serial.println("  1 dakika boyunca UDP veri gonderimi basliyor");
    Serial.println("  (5 sn'de bir JSON paketi gonderilecek)");
    printSeparator();
    Serial.println();
}
...
void loop() {
    ...
    if (millis() - lastSend >= 5000) {
        ...
        if (!gsm.udpSend(json)) {
            Serial.println("[FAIL] Gonderilemedi — UDP kesildi!");
            gsm.closeGPRS();
            while (1) delay(1000);
        }

        String rx = gsm.udpReceive(3000);
        if (rx.length() > 0) {
            rx.trim();
            Serial.print("  -> Yanit: ");
            Serial.println(rx);
