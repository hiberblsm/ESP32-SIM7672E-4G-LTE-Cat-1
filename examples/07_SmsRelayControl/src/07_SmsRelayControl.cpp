/*
 * 07_SmsRelayControl.cpp — ESP32-S3 + SIM7672E SMS Röle Kontrolü
 *
 * Yalnızca SMS röle kontrolü için gerekli fonksiyonları içerir.
...
SmsRelay::SmsRelay(uint8_t txPin, uint8_t rxPin, uint8_t pwrKeyPin, uint8_t resetPin)
...
String SmsRelay::sendAT(const String &cmd, uint32_t timeoutMs) {
    clearBuffer();
    _serial->println(cmd);
...
bool SmsRelay::sendATExpect(const String &cmd, const String &expected,
                            uint32_t timeoutMs) {
    return sendAT(cmd, timeoutMs).indexOf(expected) >= 0;
}

bool SmsRelay::waitForResponse(const String &expected, uint32_t timeoutMs) {
    char   buf[AT_BUF_SIZE];
    size_t pos = 0;
    unsigned long start = millis();
...
bool SmsRelay::smsSend(const String &number, const String &text) {
...
String SmsRelay::smsList(const String &type) {
    sendAT("AT+CMGF=1", 3000);
    return sendAT("AT+CMGL=\"" + type + "\"", SMS_LIST_TIMEOUT);
}

String SmsRelay::smsRead(int index) {
    sendAT("AT+CMGF=1", 3000);
    return sendAT("AT+CMGR=" + String(index), 5000);
}

void SmsRelay::smsSetIncoming(bool enabled) {
    if (enabled) {
        sendAT("AT+CMGF=1", 3000);
...
String SmsRelay::smsWaitIncoming(uint32_t timeoutMs) {
    String buf;
    unsigned long start = millis();
    while (millis() - start < timeoutMs) {
...
bool SmsRelay::smsDeleteAll() {
    return sendATExpect("AT+CMGD=1,4", "OK", 10000);
}

// ==================== SMS POLL (GÖNDEREN + KOMUT) ====================

bool SmsRelay::smsPoll(char *sender, size_t senderLen, char *cmd, size_t cmdLen) {
...
void SmsRelay::debugPrint(const String &msg) {
    if (_debugEnabled) {
...
void SmsRelay::setDebug(bool enabled) {
