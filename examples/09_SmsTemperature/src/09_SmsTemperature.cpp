/*
 * 09_SmsTemperature.cpp — ESP32-S3 + SIM7672E SMS Sıcaklık Ölçümü
 *
 * Yalnızca SMS sıcaklık bildirimi için gerekli fonksiyonları içerir.
...
SmsTemperature::SmsTemperature(uint8_t txPin, uint8_t rxPin, uint8_t pwrKeyPin, uint8_t resetPin)
...
bool SmsTemperature::begin(long baud) {
    if (_serial) {
        _serial->end();
        delay(30);
    }
...
String SmsTemperature::sendAT(const String &cmd, uint32_t timeoutMs) {
    clearBuffer();
    _serial->println(cmd);
...
bool SmsTemperature::sendATExpect(const String &cmd, const String &expected,
                                  uint32_t timeoutMs) {
    return sendAT(cmd, timeoutMs).indexOf(expected) >= 0;
}

bool SmsTemperature::waitForResponse(const String &expected, uint32_t timeoutMs) {
    char   buf[AT_BUF_SIZE];
    size_t pos = 0;
    unsigned long start = millis();
...
void SmsTemperature::clearBuffer() {
...
void SmsTemperature::_exitDataModeAndDrain() {
    clearBuffer();
    _serial->write(0x1B);
    delay(250);
    clearBuffer();
...
int SmsTemperature::getSignalQuality() {
...
bool SmsTemperature::isRegistered() {
...
String SmsTemperature::getSIMStatus() {
...
String SmsTemperature::getOperator() {
...
bool SmsTemperature::smsSend(const String &number, const String &text) {
    if (number.length() < 3) return false;

    delay(300);   // Önceki işlem/URC'nin tamamlanması için bekle
...
String SmsTemperature::smsList(const String &type) {
    sendAT("AT+CMGF=1", 3000);
    return sendAT("AT+CMGL=\"" + type + "\"", SMS_LIST_TIMEOUT);
}

String SmsTemperature::smsRead(int index) {
    sendAT("AT+CMGF=1", 3000);
    return sendAT("AT+CMGR=" + String(index), 5000);
}

void SmsTemperature::smsSetIncoming(bool enabled) {
    if (enabled) {
        sendAT("AT+CMGF=1", 3000);
...
String SmsTemperature::smsWaitIncoming(uint32_t timeoutMs) {
    String buf;
    unsigned long start = millis();
    while (millis() - start < timeoutMs) {
...
bool SmsTemperature::smsDeleteAll() {
    return sendATExpect("AT+CMGD=1,4", "OK", 10000);
}

// ==================== SMS POLL (GÖNDEREN + KOMUT) ====================

bool SmsTemperature::smsPoll(char *sender, size_t senderLen, char *cmd, size_t cmdLen) {
    // Non-blocking: +CMT: URC'sini kısa süre topla, yoksa hemen dön.
    String buf;
    unsigned long start = millis();
    while (millis() - start < 200) {
        while (_serial->available()) buf += (char)_serial->read();
        if (buf.indexOf("+CMT:") >= 0) break;
        yield();
    }
    if (buf.indexOf("+CMT:") < 0) return false;
    ...
    int idx = buf.indexOf("+CMT:");
    // +CMT: "numara",,"tarih"  → numara ilk tırnak çifti arasında
    int q1 = buf.indexOf('"', idx);
    int q2 = buf.indexOf('"', q1 + 1);
    if (q1 < 0 || q2 < 0) return false;
    String num = buf.substring(q1 + 1, q2);

    // Mesaj gövdesi: ilk satır sonundan itibaren
    int nl = buf.indexOf('\n', q2);
    if (nl < 0) return false;
    String msg = buf.substring(nl + 1);
    msg.trim();

    if (num.length() == 0 || msg.length() == 0) return false;

    strncpy(sender, num.c_str(), senderLen - 1);
    sender[senderLen - 1] = '\0';
    strncpy(cmd, msg.c_str(), cmdLen - 1);
    cmd[cmdLen - 1] = '\0';
    return true;
}
...
void SmsTemperature::debugPrint(const String &msg) {
    if (_debugEnabled) {
...
void SmsTemperature::setDebug(bool enabled) {
